...
re_pasted_section="${re_anchor}?^# PasteTest\\n+${re_pasted_content}"


output="$(conv_sample1 --paste-section-before DESCRIPTION:test/samples/PasteTest.md)"
assertRegex "$output" "/${re_pasted_section}${re_anchor}?# Description/m"
...
assertRegex "$output" "/^${re_pasted_content}${re_anchor}?# Description/m"

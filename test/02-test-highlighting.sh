...
B='\*\*'
I='\*'
KEEP_B="(?:\\s+|${B}\\s+${B})"  # space between two .B areas, we don't care if the space itself it bold as well
KEEP_I="(?:\\s+|${I}\\s+${I})"  # space between two .I areas, we don't care if the space itself is italic as well
BIS="(?:\\s+${B}\\s*${I}\\s*|\\s*${B}\\s+${I}\\s*|\\s*${B}\\s*${I}\\s+)"  # change from bold to italics, with at least one space
IBS="(?:\\s+${I}\\s*${B}\\s*|\\s*${I}\\s+${B}\\s*|\\s*${I}\\s*${B}\\s+)"  # change from italics to bold, with at least one space
...
assertRegex "$output" "/3\\s+${B}bold${B}\\s+3\\s+${B}bold${B}\\s+3/" \
	"Alternating highlighting (.RB, with escaped spaces in bare tokens) did not work as expected!"
assertRegex "$output" "/\\s${B}aaa +bbb +ccc${B}\\s+normal\\s+${B}bold\"quoted${KEEP_B}spaced\"bold${B}\\s+normal2\\s/" \
